* ==============================================================================
* 08_heterog_fora_semiarido.do
* Projeto : Determinantes da Adesão ao PGS – Alinhamento Político
* ------------------------------------------------------------------------------
* OBJETIVO
*   Heterogeneidade geográfica – municípios FORA do Semiárido (semiarido == 0).
*   Especificação: Probit dinâmico RE + CRE/Mundlak + condição inicial.
*
* OUTPUT : ${dir_tables}/tab_ame_probit_dyn_cre_ci_fora_semi.tex
* ==============================================================================
...
estimates store probit_dyn_cre_ci_fora
...
estimates store AME_dyn_CRE_CI_fora

* --- Exportar tabela ---------------------------------------------------------
esttab AME_dyn_CRE_CI_fora using "${dir_tables}/tab_ame_probit_dyn_cre_ci_fora_semi.tex", ///
    replace booktabs label se ///
    b(%9.3f) se(%9.3f) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    nonumber nomtitle ///
    stats(N, fmt(%9.0fc) labels("Observações")) ///
    title("AME – Fora do Semiárido (Wooldridge 2005)\label{tab:heterog_fora_semi}") ///
    addnotes("Amostra restrita a municípios fora do Semiárido (semiarido==0)." ///
             "Efeitos marginais médios (AME). Erros-padrão clusterizados no município.")

display as result "tab_ame_probit_dyn_cre_ci_fora_semi.tex exportada."
